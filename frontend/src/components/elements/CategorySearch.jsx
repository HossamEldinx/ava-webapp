        ...
        debounce(async (term) => {
            ...
            try {
                const results = await searchCategories(
                    term.trim(),
                    null, // No user filtering in search
                    100, // limit
                    0 // offset
                );
                ...
                setError(
                    error.message || t("categories.categorySearch.searchFailed")
                );
                onSearchResults({
                    categories: [],
                    count: 0,
                ...
                <div className="flex-1 relative">
                    <div className="relative">
                        ...
                        <input
                            ...
                            placeholder={t(
                                "categories.categorySearch.placeholder"
                            )}
                        />
                        {searchTerm && (
                            <div className="absolute inset-y-0 right-0 pr-3 flex items-center">
                                <button
                                    ...
                                    title={t(
                                        "categories.categorySearch.clearSearch"
                                    )}
                                >
                                    <svg
                                        className="h-4 w-4"
                        ...
                        <div className="absolute z-10 mt-1 w-full bg-gray-800 shadow-lg max-h-60 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none">
                            <div className="flex items-center justify-between px-3 py-2 border-b border-gray-700">
                                <span className="text-xs font-medium text-gray-300">
                                    {t(
                                        "categories.categorySearch.recentSearches"
                                    )}
                                </span>
                                <button
                                ...
                                >
                                    {t(
                                        "categories.categorySearch.clearHistory"
                                    )}
                                </button>
                            </div>
                    ...
                    >
                        {isSearching ? (
                            <>
                                ...
                                {t("categories.categorySearch.searching")}
                            </>
                        ) : (
                            <>
                                <svg
                                ...
                                {t("categories.categorySearch.searchButton")}
                            </>
                        )}
                    </button>
                )}
